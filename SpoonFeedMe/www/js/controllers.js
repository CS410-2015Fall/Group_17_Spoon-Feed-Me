angular.module('SpoonFeedMe.controllers', ['ionic.utils'])
    ...
    .controller('SearchCtrl', function($scope, $ionicLoading, RecipeService) {
        ...
        $scope.getRecipes = function(searchTerms) {
            ...
            RecipeService.getFromSearch(searchTerms).then(function(
                recipeData) {
    ...
    .controller('RecipeDetailCtrl', function($scope, $stateParams,
        RecipeService) {

      // Initial scope values
        $scope.recipeId = $stateParams.recipeId;
        $scope.fromSavedOrSearch = $stateParams.fromSavedOrSearch;
        var payload = RecipeService.getRecipes($scope.fromSavedOrSearch)[
            $scope.recipeId];
    ...
    .controller('WalkthroughCtrl', function($scope, $stateParams, RecipeService) {
        ...
        $scope.help = function() {
            ...
            alert(
                "To go to the previous step = Say 'Back' or 'Previous'"
            );
            alert(
                "To slow down the pace of the instruction = Say 'Slower'"
            );
            alert(
                "To speed up the pace of the instruction = Say 'Faster'"
            );
        }
